...
* ```uv.lock``` has all the dependencies needed in the project in a locked file. It is autogenerated and is simiar to package-lock.json in npm based projects.
* [project.scripts] in pyproject.toml has the entry commands including ```labelme``` command. Generation of an updated ```uv.lock``` file is also achieved by updating the dependencies in this file.
* Using the command ```labelme``` runs the main() fn. in ```labelme/__main__.py```
* This ```__main__.py``` takes in any arguments and calls the ```MainWindow``` class of ```labelme/app.py``` file. This ```app.py``` is the main file which contains features based code in labelme application.
* All the buttons/widgets in this labelme app are extended classes of ```PyQt5.QtWidgets``` and ```PyQt5.QtGui```. The main functions of the app are all present in the toolbar of the app and added to the toolbar by adding it to ```self.actions.tool```
* This version of labelme project adds a "Detect" btn that can automatically detect classes from a finetuned custom RFDeTR model. It allows detection of either all the classes of the finetuned model or selected classes of the finetuned model by pressing on the ```Classes``` btn beside Detect btn in the toolbar. (Classes btn doesnt look like a dropdown btn but pressing on it will open the dropdown,GUI needs to be fixed)
* This version project also adds multiclass/multilabel classification and this can be enabled by using the right mouseclick and selecting the Classification option. Note that if the Classification option is visible, then it is active and the corresponding single/multilabel classes are added to the json file. Any boundingboxes/object detection are still maintained in the json file and thus the same annotation file can be used for both detection/classification task seamlessly.
* Segmentation using SAM2 is not worked on by me and the original code has bugs but ```labelme/shape.py``` is used to call these models from ```labelme/app.py``` file. ```shape.py``` file doesn't host the SAM2 models but it is linked to [osam](https://github.com/wkentaro/osam) github repo (another repo of the original author of labelme). 
